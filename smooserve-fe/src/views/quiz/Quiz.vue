      ...
      <form @submit.prevent="submitForm" class="w-full">
        <div>
          ...
          <div>
            <table v-if="questions[currentQuestion].type === 'multi-select'" class="w-full">
              <tr
                  ...
                  @click="updateMultiSelect(questions[currentQuestion].name, option)"
                  :class="{
                    selected:
            ...
            <table v-else-if="questions[currentQuestion].type === 'single-select'" class="w-full">
              <tr
                  class="wrapping custom-radio-label"
                  @click="updateResponse(questions[currentQuestion].name, option, questions[currentQuestion].type)"
                  :class="{
            ...
            <div v-else style="display: flex; flex-direction: row; justify-content: center; align-items: center;">
              <span class="p-float-label w-full mt-3 mx-2">
                <Textarea
                    v-model="questions[currentQuestion].response"
                    :rows="10"
                    :maxlength="questions[currentQuestion].character_limit"
                    class="w-full"
          ...
          <div class="d-flex justify-content-between my-3 mx-2">
            <Button v-if="currentQuestion > 0" type="button" label="Back" @click="prevQuestion" class="mr-2">
            </Button>
            <div v-else></div>
            <Button v-if="currentQuestion < questions.length - 1" type="button" label="Next" @click="nextQuestion" class="ml-2">
            </Button>
            <Button v-if="currentQuestion === questions.length - 1" type="submit" label="Submit">
            </Button>
          </div>
