      ...
      <ul class="list-none select-none justify-content-center">
        ...
        <li>
          <a @click="smoothScroll('#highlights')"
            ...
            <router-link :to="{ name: 'Shop' }"><i class="navbar-icon pi pi-id-card"></i><span>SMOOSERVE SHOP</span></router-link>
          </a>
        </li>
      </ul>
    </div>
      ...
      <Button
        ...
        <Button @click="handleSignOut" class="p-button-text p-button-rounded border-none font-light line-height-2 pr-3">
          <span class="icon-label-container">
            <i class="pi pi-sign-out icon-spacing"></i> <!-- Add a specific class for spacing -->
            LOG OUT
          </span>
        </Button>
        <router-link :to="{ name: 'Profile' }"><i class="pi pi-user" style="font-size: 1.3rem"></i></router-link>
...
<style>
.icon-spacing {
  margin-right: 5px; /* Adjust the right margin to add spacing between the icon and the label */
}
